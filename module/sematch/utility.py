...
class FileIO:
    ...
    def read_json_file(name):
        data = []
        with open(FileIO.filename(name),'r', encoding='UTF8') as f:
            for line in f:
    ...
    def save_json_file(name, data):
        with open(FileIO.filename(name), 'w', encoding='UTF8') as f:
            for d in data:
                json.dump(d, f)
                f.write("\n")
    ...
    def append_json_file(name, data):
        with open(FileIO.filename(name), 'a', encoding='UTF8') as f:
            for d in data:
                json.dump(d, f)
                f.write("\n")
